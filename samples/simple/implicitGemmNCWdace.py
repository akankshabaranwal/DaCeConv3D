...
def dace_conv3d(Input: dtype[d_batchsize, d_inchannels, d_outwidth+d_kdim-1] @dace.StorageType.GPU_Global,
    ...
    d_GEMM_M = (d_batchsize*d_outwidth)
    d_GEMM_N = d_outchannels
    d_GEMM_K = (d_inchannels*d_kdim)
    ...
    for cta_n, cta_m in dace.map[0:d_GEMM_N:CTAtileN, 0:d_GEMM_M:CTAtileM] @dace.ScheduleType.GPU_Device:
                ...
                for warp_n, warp_m in dace.map[0:CTAtileN:WARPtileN, 0:CTAtileM:WARPtileM]@dace.ScheduleType.GPU_ThreadBlock:
                    for warp_k in dace.map[0:CTAtileK]@dace.ScheduleType.Sequential:
                        for gemm_n, gemm_m in dace.map[0:WARPtileN, 0:WARPtileM]@dace.ScheduleType.Sequential:
                            ...
                            n = dace.int32((gemm_m+cta_m+warp_m)/d_outwidth)
                            q = dace.int32((gemm_m+cta_m+warp_m)%d_outwidth)

                            c  = dace.int32((cta_k+warp_k)/d_kdim)
                            s  = dace.int32((cta_k+warp_k)%d_kdim)

                            w = q + s

                            cta_input[warp_k , warp_m + gemm_m] = Input[n, c, w]
                            cta_kernel[warp_k , warp_n + gemm_n] = kernel[gemm_n+cta_n+warp_n, c, s]
            ...
            for warp_n, warp_m in dace.map[0: CTAtileN:WARPtileN, 0: CTAtileM:WARPtileM]@dace.ScheduleType.GPU_ThreadBlock:
                for gemm_n, gemm_m in dace.map[0:WARPtileN, 0:WARPtileM]@dace.ScheduleType.Sequential:
                    ...
                    Output[ n, cta_n+gemm_n+warp_n, q ] = cta_reducedk[gemm_n+warp_n, gemm_m+warp_m]